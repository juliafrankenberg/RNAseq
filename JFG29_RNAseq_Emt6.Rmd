...
results.tables <- "results/R/tables/all_samples"
results.plots <- "results/R/plots/all_samples"

dir.create(results.tables, showWarnings = F, recursive = T)
dir.create(results.plots, showWarnings = F, recursive = T)

counts <- "data/R/raw_counts.csv"
design <- "data/R/design.csv"
...
Data should be normalised to control for sequencing depth, gene lenght and, optionally, variance.
...
pca_data + 
  geom_text(aes(label = name), 
            position = nudge) + ggtitle("Histology Number")
...
```
Compare to results of Judith's data
...
plot <- ggboxplot(merged_table, x = "condition", y = "GVSA_Score", add = c("dotplot"),color = "condition", ylab = "gsva score", facet.by = "Cell_type", title = "Consensus TME of E0771 and Emt6 experiments")  +
  theme(axis.text.x = element_blank(),  
  axis.title.x = element_blank()) 
...
desired_order <- c("noDOX_emt6", "DOX_emt6", "noDOX_e0771", "DOX_e0771")

merged_table$condition <- factor(merged_table$condition, levels = desired_order)


custom_palette <- c("noDOX_emt6" = "red", "DOX_emt6" = "blue", "noDOX_e0771" = "red", "DOX_e0771" = "blue")


plot <- ggboxplot(merged_table, x = "condition", y = "GVSA_Score", 
                  add = c("dotplot"), facet.by = "Cell_type", color = "condition", title = "Consensus TME of E0771 and Emt6 experiments") +
